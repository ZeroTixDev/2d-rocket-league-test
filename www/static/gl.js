let clipboard, plugins, wasm_memory, high_dpi, FS, GL, Module, wasm_exports, emscripten_shaders_hack, importObject;
('use strict');
...
function acquireVertexArrayObjectExtension(a) {
   const b = a.getExtension('OES_vertex_array_object');
   b
...
function acquireInstancedArraysExtension(a) {
   const b = a.getExtension('ANGLE_instanced_arrays');
   b &&
...
function acquireDisjointTimerQueryExtension(a) {
   const b = a.getExtension('EXT_disjoint_timer_query');
   b &&
...
function UTF8ToString(i, j) {
   let b, h, c, a, d, f, g;
   const e = new Uint8Array(wasm_memory.buffer, i);
   for (b = 0, h = b + j, c = ''; !(b >= h); ) {
      if (((a = e[b++]), !a)) return c;
      if (!(a & 128)) {
      ...
      (f = e[b++] & 63),
         ...
         a < 65536
            ? (c += String.fromCharCode(a))
            : ((g = a - 65536), (c += String.fromCharCode(55296 | (g >> 10), 56320 | (g & 1023))));
   }
   return c;
}
function stringToUTF8(f, c, b, g) {
   for (var h = b, e = b + g, d = 0, a, i; d < f.length; ++d)
      if (
         ((a = f.charCodeAt(d)),
         a >= 55296 && a <= 57343 && ((i = f.charCodeAt(++d)), (a = (65536 + ((a & 1023) << 10)) | (i & 1023))),
         a <= 127)
      ) {
         if (b >= e) break;
         c[b++] = a;
         ...
         if (b + 2 >= e) break;
         (c[b++] = 224 | (a >> 12)), (c[b++] = 128 | ((a >> 6) & 63)), (c[b++] = 128 | (a & 63));
      } else {
         if (b + 3 >= e) break;
         a >= 2097152 &&
            console.warn(
...
(FS = { loaded_files: [], unique_id: 0 }),
   (GL = {
      ...
      validateGLObjectID: function (b, a, c, d) {
         a != 0 &&
            (b[a] === null
               ? console.error(c + ' called with an already deleted ' + d + ' ID ' + a + '!')
               : b[a] || console.error(c + ' called with an invalid ' + d + ' ID ' + a + '!'));
      },
      ...
      populateUniformTable: function (i) {
         ...
         for (g = 0; g < k; ++g)
            if (
               ...
               (c = GL.getNewId(GL.uniforms)), (j[a] = [e.size, c]), (GL.uniforms[c] = b);
               for (f = 1; f < e.size; ++f)
                  (l = a + '[' + f + ']'),
      ...
      (a = gl[d]()),
         ...
         a
            ...
            : (console.error('GL_INVALID_OPERATION'),
              GL.recordError(1282),
              alert(
                 'GL_INVALID_OPERATION in ' + f + ': GLctx.' + d + ' returned null - most likely GL context is lost!'
              )),
...
function _webglGet(d, e, c) {
   let a, f, b, g;
   if (!e) {
      console.error('GL_INVALID_VALUE in glGet' + c + 'v(name=' + d + ': Function called with null out pointer!'),
         GL.recordError(1281);
      return;
   }
   switch (((a = void 0), d)) {
      ...
      case 36344:
         c != 'EM_FUNC_SIG_PARAM_I' &&
            ...
            (GL.recordError(1280),
            err('GL_INVALID_ENUM in glGet' + c + 'v(GL_SHADER_BINARY_FORMATS): Invalid parameter type!'));
         return;
      case 34814:
      case 36345:
         a = 0;
   ...
   if (a === void 0)
      switch (((b = gl.getParameter(d)), typeof b)) {
         ...
         case 'string':
            GL.recordError(1280),
               console.error('GL_INVALID_ENUM in glGet' + c + 'v(' + d + ') on a name which returns a string!');
            return;
         case 'object':
            if (b === null)
               switch (d) {
                  ...
                  default: {
                     GL.recordError(1280),
                        console.error('GL_INVALID_ENUM in glGet' + c + 'v(' + d + ') and it returns null!');
                     return;
                  }
               }
            else if (
...
function resize(a, d) {
   const e = dpi_scale(),
      b = a.clientWidth * e,
      c = a.clientHeight * e;
   (a.width != b || a.height != c) && ((a.width = b), (a.height = c), d != void 0 && d(Math.floor(b), Math.floor(c)));
}
function animation() {
...
function texture_size(c, a, b) {
   return c == gl.ALPHA ? a * b : c == gl.RGB ? a * b * 3 : c == gl.RGBA ? a * b * 4 : a * b * 3;
}
   ...
   (importObject = {
      env: {
         ...
         glBindTexture: function (b, a) {
            GL.validateGLObjectID(GL.textures, a, 'glBindTexture', 'texture'), gl.bindTexture(b, GL.textures[a]);
         },
         glTexImage2D: function (e, f, a, b, c, g, h, i, d) {
            gl.texImage2D(e, f, a, b, c, g, h, i, d ? getArray(d, Uint8Array, texture_size(a, b, c)) : null);
         },
         glTexSubImage2D: function (g, e, i, f, c, d, a, h, b) {
            gl.texSubImage2D(g, e, i, f, c, d, a, h, b ? getArray(b, Uint8Array, texture_size(a, c, d)) : null);
         },
         glReadPixels: function (d, e, a, b, c, f, g) {
            const h = getArray(g, Uint8Array, texture_size(c, a, b));
         ...
         glUniform1fv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform1fv', 'location'),
               assert((b & 3) == 0, 'Pointer to float data passed to glUniform1fv must be aligned to four bytes!');
            const d = getArray(b, Float32Array, 1 * c);
            gl.uniform1fv(GL.uniforms[a], d);
         },
         glUniform2fv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform2fv', 'location'),
               assert((b & 3) == 0, 'Pointer to float data passed to glUniform2fv must be aligned to four bytes!');
            const d = getArray(b, Float32Array, 2 * c);
            gl.uniform2fv(GL.uniforms[a], d);
         },
         glUniform3fv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform3fv', 'location'),
               assert((b & 3) == 0, 'Pointer to float data passed to glUniform3fv must be aligned to four bytes!');
            const d = getArray(b, Float32Array, 3 * c);
            gl.uniform3fv(GL.uniforms[a], d);
         },
         glUniform4fv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform4fv', 'location'),
               assert((b & 3) == 0, 'Pointer to float data passed to glUniform4fv must be aligned to four bytes!');
            const d = getArray(b, Float32Array, 4 * c);
            gl.uniform4fv(GL.uniforms[a], d);
         },
         glUniform1iv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform1fv', 'location'),
               assert((b & 3) == 0, 'Pointer to i32 data passed to glUniform1iv must be aligned to four bytes!');
            const d = getArray(b, Int32Array, 1 * c);
            gl.uniform1iv(GL.uniforms[a], d);
         },
         glUniform2iv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform2fv', 'location'),
               assert((b & 3) == 0, 'Pointer to i32 data passed to glUniform2iv must be aligned to four bytes!');
            const d = getArray(b, Int32Array, 2 * c);
            gl.uniform2iv(GL.uniforms[a], d);
         },
         glUniform3iv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform3fv', 'location'),
               assert((b & 3) == 0, 'Pointer to i32 data passed to glUniform3iv must be aligned to four bytes!');
            const d = getArray(b, Int32Array, 3 * c);
            gl.uniform3iv(GL.uniforms[a], d);
         },
         glUniform4iv: function (a, c, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform4fv', 'location'),
               assert((b & 3) == 0, 'Pointer to i32 data passed to glUniform4iv must be aligned to four bytes!');
            const d = getArray(b, Int32Array, 4 * c);
         ...
         glUniform1f: function (a, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform1f', 'location'), gl.uniform1f(GL.uniforms[a], b);
         },
         glUniform1i: function (a, b) {
            GL.validateGLObjectID(GL.uniforms, a, 'glUniform1i', 'location'), gl.uniform1i(GL.uniforms[a], b);
         },
         ...
         glGetUniformLocation: function (e, a) {
            ...
            return (
               ...
               (b = 0),
               a[a.length - 1] == ']' &&
                  ((c = a.lastIndexOf('[')), (b = a[c + 1] != ']' ? parseInt(a.slice(c + 1)) : 0), (a = a.slice(0, c))),
         ...
         glBindBuffer: function (b, a) {
            GL.validateGLObjectID(GL.buffers, a, 'glBindBuffer', 'buffer'), gl.bindBuffer(b, GL.buffers[a]);
         },
         ...
         glCreateProgram: function () {
            const a = GL.getNewId(GL.programs),
               b = gl.createProgram();
         ...
         glFramebufferTexture2D: function (b, c, d, a, e) {
            GL.validateGLObjectID(GL.textures, a, 'glFramebufferTexture2D', 'texture'),
               gl.framebufferTexture2D(b, c, d, GL.textures[a], e);
         },
         glGetProgramiv: function (a, b, c) {
            let e, d;
            if ((assert(c), GL.validateGLObjectID(GL.programs, a, 'glGetProgramiv', 'program'), a >= GL.counter)) {
            ...
            if (b == 35716)
               ...
               console.error('unsupported operation');
               return;
            } else getArray(c, Int32Array, 1)[0] = gl.getProgramParameter(GL.programs[a], b);
         },
         glCreateShader: function (b) {
            const a = GL.getNewId(GL.shaders);
         ...
         glShaderSource: function (b, d, e, f) {
            ...
            GL.validateGLObjectID(GL.shaders, b, 'glShaderSource', 'shader'),
               ...
               emscripten_shaders_hack &&
                  ((a = a.replace(/#extension GL_OES_standard_derivatives : enable/g, '')),
                  (a = a.replace(/#extension GL_EXT_shader_texture_lod : enable/g, '')),
                  (c = ''),
                  a.indexOf('gl_FragColor') != -1 &&
                     ((c += 'out mediump vec4 GL_FragColor;\n'), (a = a.replace(/gl_FragColor/g, 'GL_FragColor'))),
                  a.indexOf('attribute') != -1
                     ? ((a = a.replace(/attribute/g, 'in')), (a = a.replace(/varying/g, 'out')))
         ...
         glGetProgramInfoLog: function (d, c, g, e) {
            ...
            GL.validateGLObjectID(GL.programs, d, 'glGetProgramInfoLog', 'program'),
               ...
               assert(b !== null);
            const f = getArray(e, Uint8Array, c);
            for (a = 0; a < c; a++) f[a] = b.charCodeAt(a);
         },
         glCompileShader: function (a, b, c, d) {
            GL.validateGLObjectID(GL.shaders, a, 'glCompileShader', 'shader'), gl.compileShader(GL.shaders[a]);
         },
         glGetShaderiv: function (a, d, b) {
            ...
            assert(b),
               ...
               d == 35716
                  ...
                  : (getArray(b, Int32Array, 1)[0] = gl.getShaderParameter(GL.shaders[a], d));
         },
         ...
         glDeleteBuffers: function (d, e) {
            for (var a = 0, c, b; a < d; a++) {
               if (((c = getArray(e + a * 4, Uint32Array, 1)[0]), (b = GL.buffers[c]), !b)) continue;
               gl.deleteBuffer(b), (b.name = 0), (GL.buffers[c] = null);
            }
         },
         glDeleteFramebuffers: function (d, e) {
            for (var a = 0, c, b; a < d; a++) {
               if (((c = getArray(e + a * 4, Uint32Array, 1)[0]), (b = GL.framebuffers[c]), !b)) continue;
               gl.deleteFramebuffer(b), (b.name = 0), (GL.framebuffers[c] = null);
            }
         },
         glDeleteTextures: function (d, e) {
            for (var a = 0, c, b; a < d; a++) {
               if (((c = getArray(e + a * 4, Uint32Array, 1)[0]), (b = GL.textures[c]), !b)) continue;
         ...
         glDeleteQueries: function (d, e) {
            for (var a = 0, c, b; a < d; a++) {
               if (((c = getArray(textures + a * 4, Uint32Array, 1)[0]), (b = GL.timerQueries[c]), !b)) continue;
               gl.deleteQuery(b), (b.name = 0), (GL.timerQueries[c] = null);
            }
         },
         glBeginQuery: function (b, a) {
            GL.validateGLObjectID(GL.timerQueries, a, 'glBeginQuery', 'id'), gl.beginQuery(b, GL.timerQueries[a]);
         },
         ...
         glGetQueryObjectiv: function (a, b, c) {
            ...
            const d = gl.getQueryObject(GL.timerQueries[a], b);
            getArray(c, Uint32Array, 1)[0] = d;
         },
         glGetQueryObjectui64v: function (b, d, e) {
            GL.validateGLObjectID(GL.timerQueries, b, 'glGetQueryObjectui64v', 'id');
            const c = gl.getQueryObject(GL.timerQueries[b], d),
         ...
         run_animation_loop: function (a) {
            (canvas.onmousemove = function (a) {
               const b = mouse_relative_position(a.clientX, a.clientY),
                  c = b.x,
                  d = b.y;
               wasm_exports.mouse_move(Math.floor(c), Math.floor(d)),
                  (a.movementX != 0 || a.movementY != 0) &&
                     wasm_exports.raw_mouse_move(Math.floor(a.movementX), Math.floor(a.movementY));
            }),
               ...
               (canvas.onkeydown = function (a) {
                  let b = into_sapp_keycode(a.code),
                     c;
                  switch (b) {
                     case 32:
                     case 262:
                  ...
                  (c = 0),
                     ...
                     b == 32 && wasm_exports.key_press(b);
               }),
               (canvas.onkeyup = function (a) {
                  const b = into_sapp_keycode(a.code);
                  wasm_exports.key_up(b);
               }),
               (canvas.onkeypress = function (a) {
                  const b = into_sapp_keycode(a.code);
                  const c = b == 261 || a.ctrlKey;
               ...
               canvas.addEventListener('touchstart', function (a) {
                  ...
                  for (const b of a.changedTouches)
                     wasm_exports.touch(
                        ...
                        b.identifier,
                        Math.floor(b.clientX),
                        Math.floor(b.clientY)
                     );
               }),
               ...
               window.addEventListener('copy', function (a) {
                  clipboard != null && (event.clipboardData.setData('text/plain', clipboard), event.preventDefault());
               }),
         ...
         fs_take_buffer: function (c, e, d) {
            let b = FS.loaded_files[c],
               f,
               a;
            console.assert(b.length <= d), (f = new Uint8Array(wasm_memory.buffer, e, d));
         ...
         sapp_set_window_size: function (a, b) {
            (canvas.width = a), (canvas.height = b), resize(canvas, wasm_exports.resize);
         },
      },
   });
function register_plugins(a) {
   ...
   for (let b = 0; b < a.length; b++)
      a[b].register_plugin != void 0 && a[b].register_plugin != null && a[b].register_plugin(importObject);
}
...
function init_plugins(b) {
   ...
   for (a = 0; a < b.length; a++)
      b[a].on_init != void 0 && b[a].on_init != null && b[a].on_init(),
         b[a].name == void 0 || b[a].name == null || b[a].version == void 0 || b[a].version == null
            ...
            : ((c = b[a].name + '_crate_version'),
              wasm_exports[c] == void 0
                 ? console.error(
                      'Plugin ' + b[a].name + ' miss version function: ' + c + '. Probably invalid crate version.'
                   )
...
function add_missing_functions_stabs(b) {
   const a = WebAssembly.Module.imports(b);
   for (const b in a)
   ...
   register_plugins(plugins),
      typeof WebAssembly.compileStreaming == 'function'
         ? WebAssembly.compileStreaming(a)
              .then((a) => (add_missing_functions_stabs(a), WebAssembly.instantiate(a, importObject)))
              .then((a) => {
                 (wasm_memory = a.exports.memory), (wasm_exports = a.exports);
                 const b = u32_to_semver(wasm_exports.crate_version());
                 version != b &&
                    console.error(
                       'Version mismatch: gl.js version is: ' + version + ', rust sapp-wasm crate version is: ' + b
                    ),
                    init_plugins(plugins),
                    a.exports.main();
              })
              .catch((a) => {
                 console.error('WASM failed to load, probably incompatible gl.js version'), console.error(a);
              })
         : a
              .then(function (a) {
              ...
              .then(function (a) {
                 return add_missing_functions_stabs(a), WebAssembly.instantiate(a, importObject);
              })
              .then(function (a) {
                 (wasm_memory = a.exports.memory), (wasm_exports = a.exports);
                 const b = u32_to_semver(wasm_exports.crate_version());
